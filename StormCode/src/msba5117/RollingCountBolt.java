...
public class RollingCountBolt extends BaseRichBolt {
  ...
  private void emitCurrentWindowCounts() {
    Map<Object,String> geo = counter.getGeoThenAdvanceWindow();
    ...
    emit(counts,geo, actualWindowLengthInSeconds);
  }

  private void emit(Map<Object, Long> counts, Map<Object, String> geo, int actualWindowLengthInSeconds) {
    for (Entry<Object, Long> entry : counts.entrySet()) {
      Object obj = entry.getKey();
      Long count = entry.getValue();
      String location = geo.get(entry.getKey());
      //Integer intCount = count != null ? count.intValue() : null;
      collector.emit(new Values(obj, count, location,actualWindowLengthInSeconds));
    }
  }
  ...
  private void countObjAndAck(Tuple tuple) {
    ...
    Object obj = word;
    String geolocation = tuple.getString(0).split(TweetTopology.DELIMITER)[1];
    Object geo = geolocation;
    counter.incrementCount(obj,geo);
    collector.ack(tuple);
  }

  @Override
  public void declareOutputFields(OutputFieldsDeclarer declarer) {
    declarer.declare(new Fields("word", "count", "location", "actualWindowLengthInSeconds"));
  }
